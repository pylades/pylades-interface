import { PYL } from './../../constants/index'
import { TokenAmount, JSBI, ChainId } from '@pyladesprotocol/sdk'
...
function fetchClaim(account: string, chainId: ChainId): Promise<UserClaimData | null> {
  ...
  return (CLAIM_PROMISES[key] =
    CLAIM_PROMISES[key] ??
    fetch(`https://gentle-frost-9e74.pylades.workers.dev/${chainId}/${formatted}`)
      .then(res => {
        if (res.status === 200) {
          return res.json()
        } else {
...
// check if user is in blob and has not yet claimed PYL
      ...
      return distributorContract
        ...
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claimed ${unClaimedAmount?.toSignificant(4)} PYL`,
